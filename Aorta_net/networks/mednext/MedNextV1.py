...
class MedNeXt1(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_0 = OutBlock(in_channels=n_channels, n_classes=n_channels, dim=dim)
        ...
        if deep_supervision:
            ...
            self.out_1 = OutBlock(in_channels=n_channels*2, n_classes=n_channels*2, dim=dim)
            self.out_2 = OutBlock(in_channels=n_channels*4, n_classes=n_channels*4, dim=dim)
            self.out_3 = OutBlock(in_channels=n_channels*8, n_classes=n_channels*8, dim=dim)
            self.out_4 = OutBlock(in_channels=n_channels*16, n_classes=320, dim=dim)
            #
            self.cov_out_1 = nn.Conv3d(in_channels=n_channels*2, out_channels=n_classes,kernel_size=1)
            self.cov_out_2 = nn.Conv3d(in_channels=n_channels*4, out_channels=n_classes,kernel_size=1)
            self.cov_out_3 = nn.Conv3d(in_channels=n_channels*8, out_channels=n_classes,kernel_size=1)
            self.cov_out_4 = nn.Conv3d(in_channels=320, out_channels=n_classes,kernel_size=1)
        self.block_counts = block_counts
...
class MedNeXt2(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            x = self.decoder3(x, x_res_3)
            x = self.decoder2(x, x_res_2)
            x = self.decoder1(x, x_res_1)
            x = self.decoder0(x, x_res_0)
            x = self.decoder(x)

            # x = checkpoint.checkpoint(self.decoder3, (x,x_res_3), self.dummy_tensor)
            # x = checkpoint.checkpoint(self.decoder2, (x, x_res_2), self.dummy_tensor)
            # x = checkpoint.checkpoint(self.decoder1, (x, x_res_1), self.dummy_tensor)
            # x = checkpoint.checkpoint(self.decoder0, (x, x_res_0), self.dummy_tensor)
