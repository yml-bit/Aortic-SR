...
class Aorta1(nn.Module):
    ...
    def __init__(self, 
        ...
        #     nn.Conv3d(in_channels, n_channels, kernel_size=7, stride=2, padding=3),
        #     nn.GroupNorm(n_channels, eps=1e-6, data_format="channels_first")
        # )
        if type(exp_r) == int:
        ...
        # self.bottleneck = nn.Sequential(*[
        #     DHDC_module(n_channels*16, n_channels*16, self.activation)
        #     for i in range(block_counts[4])]
        # )

        # self.bottleneck = DHDC_module(n_channels * 16, n_channels * 16, self.activation)
        ...
        self.Cascade_attention=Cascade_attention(8,1)
...
class Aorta2(nn.Module):
    ...
    def __init__(self,
        ...
        self.stem = conv(in_channels, n_channels, kernel_size=1)
        # self.stem = nn.Sequential(
        #     nn.Conv3d(in_channels, n_channels, kernel_size=4, stride=1, padding=2),
        #     nn.GroupNorm(num_groups=n_channels,num_channels=n_channels),
        #     # LayerNorm(n_channels, eps=1e-6, data_format="channels_first")
        # )
        if type(exp_r) == int:
            exp_r = [exp_r for i in range(len(block_counts))]
        ...
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
        if deep_supervision:
            self.Cascade_attention1 = Cascade_attention(n_channels//2, 1)
            self.out_1 = OutBlock(in_channels=n_channels * 2, n_classes=n_classes, dim=dim)
            self.Cascade_attention2 = Cascade_attention(n_channels, 1)
            self.out_2 = OutBlock(in_channels=n_channels * 4, n_classes=n_classes, dim=dim)
            self.Cascade_attention3 = Cascade_attention(n_channels*2, 1)
            self.out_3 = OutBlock(in_channels=n_channels * 8, n_classes=n_classes, dim=dim)
            self.Cascade_attention4 = Cascade_attention(n_channels*4, 1)
            self.out_4 = OutBlock(in_channels=n_channels * 16, n_classes=n_classes, dim=dim)
        self.block_counts = block_counts
